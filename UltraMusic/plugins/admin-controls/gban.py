...
from UltraMusic import app, db, lang, userbot
from UltraMusic.helpers import phrase_command


@app.on_message(phrase_command(["حظر عالمي"]) & app.sudo_filter)
@lang.language()
async def _gban(_, m: types.Message):
    ...
    # Check if user ID provided
        ...
        try:
            ...
            return await m.reply_text(m.lang["gban_invalid_id"])
        except Exception:
            return await m.reply_text(m.lang["gban_user_not_found"])
    else:
    ...
    if user_id in app.sudoers:
        return await m.reply_text(m.lang["gban_cannot_ban_sudo"])
...
@app.on_message(phrase_command(["الغاء الحظر العالمي"]) & app.sudo_filter)
@lang.language()
...
@app.on_message(phrase_command(["قائمة الحظر العالمي"]) & app.sudo_filter)
@lang.language()
async def _gbanlist(_, m: types.Message):
    ...
    sent = await m.reply_text(m.lang["gban_fetching"])
    
    gbanned = await db.get_gbanned()
    
    if not gbanned:
        return await sent.edit_text(m.lang["gban_no_banned"])
